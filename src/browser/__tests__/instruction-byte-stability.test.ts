...
function encodeMethodInstruction(
  name: string,
...
describe("instruction byte stability", () => {
  test("keeps discriminator hex values byte-identical", () => {
    ...
    expect(hexFromBytes(instructionDiscriminator("cancel_plan_change"))).toBe(
      "37d0c8f18612c0a5",
    );
  });
  ...
  test("preserves create, cancel-plan-change, cancel, and wrap+subscribe instruction data", async () => {
    ...
    const { instruction: createInstruction } = await buildCreatePlanInstruction(
      program,
      {
        merchant,
        planId: 7n,
    ...
    const { instruction: cancelInstruction } = await buildCancelInstruction(
      program,
      {
        authority,
    ...
    expect(hexFromBytes(cancelPlanChange.instruction.data)).toBe(
      "37d0c8f18612c0a5",
    );
    expect(hexFromBytes(cancelInstruction.data)).toBe("e8dbdf29dbecdcbe");
    expect(
      wrapAndSubscribe.instructions.map((instruction) =>
        hexFromBytes(instruction.data),
      ),
    ).toEqual([
      "fe1cbf8a9cb3b735",
      hexFromBytes(
    ...
    expect(
      wrapAndSubscribe.instructions[1]?.programId.equals(
        ASSOCIATED_TOKEN_PROGRAM_ID,
      ),
    ).toBe(true);
    ...
    expect(wrapAndSubscribe.instructions[0]?.programId.equals(authority)).toBe(
      true,
    );
    expect(wrapAndSubscribe.instructions[2]?.programId.equals(authority)).toBe(
      true,
    );
  });
});
