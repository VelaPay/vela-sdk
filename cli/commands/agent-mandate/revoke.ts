...
export function registerAgentMandateRevoke(parent: Command): void {
  parent
    .command("revoke")
    .description("Revoke an agent mandate and reclaim remaining funds")
    ...
    .option(
      ...
      "Destination SPL USDC account for reclaimed funds",
    )
    ...
    .action(async (opts, command: Command) => {
      try {
        ...
        const confirmed = await confirmAction(
          `Revoke agent mandate for ${agent.toBase58()}?`,
          { yes: opts.yes },
        );
        ...
        const result = await vela.revokeAgentMandate({
          agent,
